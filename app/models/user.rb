class User < ActiveRecord::Base
  ...
  def self.from_omniauth_facebook(auth)
    where(email: auth.info.email).first_or_create do |user|
      ...
      user.first_name = (auth.info.first_name || '').gsub!(/[^a-zA-Zа-яА-ЯөӨүҮ \-]|^[\- ]*/, '').strip[0...20]
      user.last_name = (auth.info.last_name || '').gsub!(/[^a-zA-Zа-яА-ЯөӨүҮ \-]|^[\- ]*/, '').strip[0...20]
  ...
  def self.from_omniauth_twitter(auth)
    where(provider: auth.provider, uid: auth.uid).first_or_create do |user|
      ...
      user.uid = auth.uid
      user.password = Devise.friendly_token[10,20]
      user.first_name = (auth.info.name || '').gsub!(/[^a-zA-Zа-яА-ЯөӨүҮ \-]|^[\- ]*/, '').strip[0...20]
